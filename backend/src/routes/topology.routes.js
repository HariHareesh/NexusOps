...
router.get("/health", async (_req, res) => {
    const result = await (0, lambda_service_1.invokeLambda)(aws_config_1.env.TOPOLOGY_LAMBDA, {
        action: "health",
    });
...
router.get("/discover", async (_req, res) => {
    ...
    (0, ws_broadcaster_1.broadcastEvent)("TOPOLOGY_UPDATED", parsed);
    res.status(result.statusCode || 200).json(parsed);
});
exports.default = router;
//# sourceMappingURL=topology.routes.js.map